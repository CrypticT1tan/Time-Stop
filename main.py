...
class Window:
    def __init__(self, window):
        ...
        self.title = Label(text="TIMER", fg="red", font=(self.font, self.title_size, "bold"),
                           width=30, bg=self.window_color)
        self.title.grid(row=0, column=0, columnspan=2)
        # Creating the Timer Icon Image
        self.timer_icon_image = Image.open(self.resource_path("timer_icon.png"))
        ...
        self.stopwatch_icon_image = Image.open(self.resource_path("stopwatch_icon.png"))
        ...
        self.canvas = Canvas(width=64, height=64, bg=self.window_color,
                                   highlightbackground=self.window_color)
        self.canvas.grid(row=1, column=0, columnspan=2)
        self.canvas.create_image(32, 36, image=self.timer_icon)
        # Creating the Entry Box for Timer
        self.timer_entry = Entry(font=(self.font, self.button_size), justify="center",
                                 highlightbackground=self.window_color)
        ...
        self.display = Label(text="00:00:00", font=(self.font, self.display_size),
                                   bg=self.window_color)
        self.display.grid(row=3, column=0, columnspan=2)
        # Creating the Reset Button for Timer
        self.reset_button = Button(text="Reset", font=(self.font, self.button_size), fg="red",
        ...
        self.startstop_button = Button(text="Start", font=(self.font, self.button_size),
        ...
        self.dropdown = OptionMenu(self.window, self.option, *self.options, command=self.switch_mode)
    ...
    def switch_mode(self, op) -> None:
        ...
        if op == "Timer" and self.title.cget("text") != "TIMER":
            ...
            self.window.title("Timer")
            self.window_color = "#f0b8b8"
            window.config(bg=self.window_color)
    ...
    def timer_count(self, sec_total) -> None:
        ...
        if self.display.cget("text") != "00:00:00":
            ...
            self.timer_stop()
            # Notify user when their timer is done
            client.create_notification(
                title="Time's Up!",
                subtitle="Your timer is done!",
                icon="timer_icon.png"
            )
            # To play the alarm sound when the timer reaches 00:00:00
            self.sound = playsound(self.resource_path("timer_alarm.mp3"), block=False)
